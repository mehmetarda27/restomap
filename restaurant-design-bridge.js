(() => {
  ...
  function showNotificationCenter() {
    ...
    modal("Bildirim Merkezi", `<div class="zg-modal-actions mb-3"><button type="button" class="zg-action" data-read-all ${(state.data?.unreadNotificationCount ?? notifications.some((item) => item.unread !== false && !item.readAt)) > 0 ? "" : "disabled"}>Tümünü Okundu İşaretle</button></div>${notifications.length ? notifications.map((item) => `<div class="zg-list-row"><div><b>${safe(item.message || item.title || "Bildirim")}</b><div class="text-xs text-slate-500">${dateTime(item.createdAt)}${item.readAt ? ` · Okundu ${dateTime(item.readAt)}` : " · Okunmadı"}</div></div></div>`).join("") : '<div class="zg-empty">Henüz bildirim yok.</div>'}`, (root) => {
  ...
  function hydrate(data) {
    ...
    if (badge) {
      const unread = data.unreadNotificationCount ?? (data.notifications || []).filter((item) => item.unread !== false && !item.readAt).length;
